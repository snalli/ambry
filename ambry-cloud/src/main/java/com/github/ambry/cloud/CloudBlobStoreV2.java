...
public class CloudBlobStoreV2 implements Store {
   ...
   */
  @Override
  public Set<StoreKey> findMissingKeys(List<StoreKey> storeKeys) throws StoreException {
   ...
   */
  @Override
  public MessageInfo findKey(StoreKey storeKey) throws StoreException {
    Timer.Context timer = null;
    try {
      timer = azureMetrics.findKeyTime.time();
      ...
      if (e instanceof BlobStorageException
        ...
        logger.error("Blob {} not found in Azure blob storage", storeKey.getID());
        throw new StoreException(e, StoreErrorCodes.ID_Not_Found);
      }
      throw new StoreException(e, StoreErrorCodes.IOError);
    } finally {
      if (timer != null) {
